...
if not lead:
    lead = Lead.objects.filter(linkedin_url=public_id_to_url(public_id)).first()
if not lead:
    print(f"No lead found for {public_id}")
    sys.exit(1)

from linkedin.db.leads import lead_to_profile_dict
profile_dict = lead_to_profile_dict(lead) or {"public_identifier": public_id, "url": public_id_to_url(public_id)}
